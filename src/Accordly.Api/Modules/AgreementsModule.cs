...
public sealed class AgreementsModule : ICarterModule
{
    ...
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("/", async (HttpContext context, ICurrentUserService currentUser, ISender sender, CancellationToken cancellationToken) => Results.Ok(await sender.Send(new GetAgreementQuery(Guid.Empty, currentUser.GetRequiredUserId(context.User)), cancellationToken)));
